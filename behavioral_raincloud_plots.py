...
all_df = pd.read_csv('{}\\all_subjects_v4.csv'.format(fpath), sep = ';')
...
subject_list = all_df['Subject'].to_list()
NC_start = subject_list.index('S0001')


patient_df = all_df.iloc[:NC_start,:]
control_df = all_df.iloc[NC_start:,:]
...
for idx, measures in enumerate(all_measures):
    ...
    if len(measures) == 2:
        ...
        print('TSDT - {}: {}'.format(measures, prob))
        y = plt.ylim()[1]
        if prob < 0.05:
            x1, x2 = -0.05, 0.05
            col = 'k'
        ...
        print('Go/NoGo - {}: {}'.format(measures, prob))
        if prob < 0.05:
            x1, x2 = 0.95, 1.05
            plt.plot([x1, x1, x2, x2], [y, y+h, y+h, y], lw=1, c=col)
            plt.text((x1+x2)*.5, y+h, '*', ha='center', va='bottom', color=col, size = 12)
